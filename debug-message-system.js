...
async function debugMessageSystem() {
  try {
    ...
    messages.forEach((message) => {
      console.log(
        `  - "${message.title}" (${message.isGlobal ? '全局' : '定向'})`
      );
      console.log(
    ...
    userMessages.forEach((userMessage) => {
      console.log(
        `  - 用户: ${userMessage.user.name} (${userMessage.user.email})`
      );
      console.log(
        `    消息: "${userMessage.message.title}" (${userMessage.message.isGlobal ? '全局' : '定向'})`
      );
    ...
    console.log(
      `使用admin用户发送测试消息: ${adminUser.name} (${adminUser.email})`
    );
