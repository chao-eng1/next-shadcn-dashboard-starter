...
export const useRealtimeTask = (projectId?: string) => {
  ...
  const broadcastTaskEdit = useCallback(
    (taskId: string) => {
      if (socket) {
  ...
  const stopTaskEdit = useCallback(
    (taskId: string) => {
      if (socket) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const optimizedTask = await aiOptimizer.optimizeTaskCreation(
      taskData,
      user.id
    );
...
export async function PATCH(
  ...
  try {
    ...
    // 发送实时更新
    await broadcastTaskUpdate(existingTask.projectId, 'task_updated', {
      taskId,
      updates
    });
...
export class TaskWorkflowEngine {
  ...
  private async analyzeDependencies(
    ...
    try {
      ...
      // 检查循环依赖
      const hasCyclicDependency = await this.checkCyclicDependency(
        task.id,
        dependencies
      );
  ...
  private async suggestAssignee(
    ...
    try {
      ...
      // 使用AI分析最佳负责人
      const assigneeSuggestion = await this.analyzeOptimalAssignee(
        task,
        project
      );
      ...
      return {
        ...state,
        error: `Assignee suggestion failed: ${error.message}`
      };
    }
  }
  ...
  private async updateMetrics(
    ...
    try {
      ...
      await this.updateProjectMetrics(project.id, {
        ...
        taskCompleted:
          state.action === 'STATUS_CHANGED' && task.status === 'DONE',
  ...
  private async estimateTaskEffort(task: any, project: any) {
    ...
    return {
      ...
      adjustedEstimate: Math.round(
        baseHours * complexityMultiplier * teamExperienceMultiplier
      ),
      confidence: 0.75,
      factors: {
  ...
  private async updateProjectMetrics(projectId: string, metrics: any) {
    ...
    await prisma.projectMetrics.upsert({
      ...
      update: {
        ...
        aiOptimizationsUsed: metrics.aiOptimizationUsed
          ? { increment: 1 }
          : undefined,
        updatedAt: new Date()
      },
...
export class TaskCacheManager {
  ...
  async cacheTaskList(
    ...
    await this.redis.setex(cacheKey, this.TTL.TASK_LIST, JSON.stringify(tasks));
  }
  ...
  private getTaskListCacheKey(projectId: string, filters: TaskFilters): string {
...
export class OptimizedTaskQueries {
  // 批量获取任务（减少N+1查询）
  async getTasksWithRelations(taskIds: string[]): Promise<Map<string, Task>> {
  ...
  async getProjectTaskStats(projectId: string): Promise<any> {
    ...
    stats.forEach((stat) => {
      ...
      result.byStatus[stat.status] =
        (result.byStatus[stat.status] || 0) + count;
      result.byPriority[stat.priority] =
        (result.byPriority[stat.priority] || 0) + count;
...
const useTaskPerformance = () => {
  ...
  const measureTaskUpdate = useCallback(
...
test.describe('任务管理', () => {
  ...
  test('应该显示AI建议', async ({ page }) => {
    ...
    // 输入任务描述
    await page.fill('[data-testid="ai-input"]', '创建用户登录功能');
