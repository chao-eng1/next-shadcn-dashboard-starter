...
async function checkCurrentUser() {
  try {
    ...
    for (const user of users) {
      ...
      if (unreadMessages.length > 0) {
        ...
        unreadMessages.forEach((userMsg, index) => {
          ...
          console.log(
            `       消息类型: ${userMsg.message.isGlobal ? '全局消息' : '定向消息'}`
          );
        });
      }
    }
    ...
    recentMessages.forEach((message, index) => {
      ...
      if (message.recipients.length > 0) {
        ...
        message.recipients.forEach((recipient) => {
          console.log(
            `    - ${recipient.user.name} (${recipient.user.email}) - 已读: ${recipient.isRead}`
          );
        });
      }
    });
    ...
    userRoles.forEach((userRole) => {
      console.log(
        `  - ${userRole.user.name} (${userRole.user.email}) -> ${userRole.role.name}`
      );
    });
    ...
    for (const user of users) {
      ...
      console.log(
        `  用户 ${user.name} 的未读消息API响应: { "unreadCount": ${unreadCount} }`
      );
    }
    ...
    for (const user of users) {
      ...
      userMessages.forEach((userMsg, index) => {
        ...
        console.log(
          `        "readAt": ${userMsg.readAt ? `"${userMsg.readAt}"` : 'null'},`
        );
        console.log('        "message": {');
        console.log(`          "id": "${userMsg.message.id}",`);
        console.log(`          "title": "${userMsg.message.title}",`);
        console.log(
          `          "content": "${userMsg.message.content.substring(0, 50)}...",`
        );
