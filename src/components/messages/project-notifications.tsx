...
interface ProjectNotification {
  ...
  title: string;
  content: string;
  type: 'task' | 'milestone' | 'deadline' | 'assignment' | 'comment' | 'status' | 'meeting';
...
export function ProjectNotifications() {
    ...
    const mockNotifications: ProjectNotification[] = [
      {
        id: '1',
        title: '任务已分配给您',
        content: '您被分配了新任务："实现用户认证模块"，截止日期为2024年1月25日。',
        type: 'assignment',
        priority: 'high',
        projectId: 'proj1',
        projectName: '电商平台开发',
        sender: {
          id: 'user1',
          name: '张项目经理',
          avatar: 'https://trae-api-sg.mchost.guru/api/ide/v1/text_to_image?prompt=professional%20avatar%20project%20manager&image_size=square',
          role: '项目经理'
        },
      ...
      },
      {
        id: '5',
        title: '新评论',
        content: '王测试在任务"用户界面测试"中添加了新评论："发现了几个UI问题，需要修复"。',
        type: 'comment',
        priority: 'medium',
        projectId: 'proj1',
        projectName: '电商平台开发',
        sender: {
          id: 'user3',
          name: '王测试',
          avatar: 'https://trae-api-sg.mchost.guru/api/ide/v1/text_to_image?prompt=professional%20avatar%20tester&image_size=square',
          role: '测试工程师'
        },
  ...
  }, [notifications, searchQuery, filterType, filterProject, filterPriority, activeTab]);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {notifications.filter(notif => !notif.isRead && !notif.isArchived).length} 条未读通知
              </p>
            </div>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-4">
          <TabsList>
            ...
            <TabsTrigger value="unread">
              未读 ({notifications.filter(notif => !notif.isRead && !notif.isArchived).length})
            </TabsTrigger>
            <TabsTrigger value="starred">
              收藏 ({notifications.filter(notif => notif.isStarred && !notif.isArchived).length})
            </TabsTrigger>
                  ...
                  {projectSummaries.map((project) => (
                    <Card
                      ...
                      className={cn(
                        ...
                        filterProject === project.id && 'ring-2 ring-primary'
                      )}
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-1 text-sm text-muted-foreground">
                              <div className="flex justify-between">
                                <span>未读通知:</span>
                                <span className="font-medium text-primary">{project.unreadCount}</span>
                              </div>
                              <div className="flex justify-between">
                                <span>总通知:</span>
                              ...
                              <div className="flex justify-between">
                                <span>最后活动:</span>
                                <span>{format(project.lastActivity, 'MM-dd HH:mm', { locale: zhCN })}</span>
                              </div>
                            </div>
                          </div>
                ...
                {filteredNotifications.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">
                      {activeTab === 'unread' ? '没有未读通知' :
                       activeTab === 'starred' ? '没有收藏的通知' :
                       activeTab === 'archived' ? '没有归档的通知' :
                       '没有找到相关通知'}
                    </p>
                  </div>
                ) : (
                            ...
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex-1">
                                ...
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                  ...
                                  <span>•</span>
                                  <span>{format(notification.timestamp, 'MM-dd HH:mm', { locale: zhCN })}</span>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center gap-1">
                                ...
                                <DropdownMenu>
                                  ...
                                  <DropdownMenuContent align="end">
                                    ...
                                    >
                                      {notification.isRead ? (
                                        <><EyeOff className="h-4 w-4 mr-2" />标为未读</>
                                      ) : (
                                        <><Eye className="h-4 w-4 mr-2" />标为已读</>
                                      )}
                                    </DropdownMenuItem>
                                    <DropdownMenuItem
                                      onClick={(e) => {
            ...
            {selectedNotification ? (
              <Card className="h-full rounded-none border-0">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2 text-sm text-muted-foreground">
                        ...
                        <Badge variant="secondary">{getTypeText(selectedNotification.type)}</Badge>
                        <div className={cn(
                          'w-2 h-2 rounded-full',
                          getPriorityColor(selectedNotification.priority)
                        )} />
                        <span>{getPriorityText(selectedNotification.priority)}</span>
                      </div>
                    </div>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  {/* 通知内容 */}
                  <div>
                    <p className="text-sm leading-relaxed">{selectedNotification.content}</p>
                  </div>
                      ...
                      <div className="space-y-2 text-sm">
                        {selectedNotification.metadata.dueDate && (
                          <div className="flex justify-between">
                            ...
                            <span>{format(selectedNotification.metadata.dueDate, 'yyyy-MM-dd', { locale: zhCN })}</span>
                          </div>
                        )}
                        {selectedNotification.metadata.assignee && (
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">分配给:</span>
                            <span>{selectedNotification.metadata.assignee}</span>
                          </div>
                        )}
                        {selectedNotification.metadata.oldStatus && selectedNotification.metadata.newStatus && (
                          <div className="flex justify-between">
                            <span className="text-muted-foreground">状态变更:</span>
                            <span>{selectedNotification.metadata.oldStatus} → {selectedNotification.metadata.newStatus}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {selectedNotification.relatedUrl && (
                    <div>
                      <Button
                        className="w-full"
                        onClick={() => {
                          if (selectedNotification.relatedUrl!.startsWith('/')) {
                            router.push(selectedNotification.relatedUrl!);
                          } else {
                            window.open(selectedNotification.relatedUrl!, '_blank');
                          }
                        }}
                      >
                  ...
                  <div className="pt-4 border-t">
                    <h4 className="font-medium mb-2">发送者</h4>
                    <div className="flex items-center gap-3">
                      <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center">
                        <User className="h-4 w-4 text-primary" />
                      </div>
                      <div>
                        <p className="text-sm font-medium">{selectedNotification.sender.name}</p>
                        <p className="text-xs text-muted-foreground">{selectedNotification.sender.role}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-4 border-t text-xs text-muted-foreground space-y-1">
                    <div className="flex justify-between">
                      <span>时间:</span>
                      <span>{format(selectedNotification.timestamp, 'yyyy-MM-dd HH:mm:ss', { locale: zhCN })}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>状态:</span>
                      <span>{selectedNotification.isRead ? '已读' : '未读'}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
