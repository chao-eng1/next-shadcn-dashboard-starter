    ...
    const fetchProjectStats = async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.warn(
            `API请求失败 (${response.status}): ${errorMessage}，使用默认统计数据`
          );
          setStats({
            totalProjects: 0,
            activeProjects: 0,
        ...
        if (result.success) {
          ...
          const trends = {
            ...
            activeTrend:
              Math.random() > 0.7 ? Math.random() * 15 : -Math.random() * 5,
            completedTrend:
              Math.random() > 0.6 ? Math.random() * 25 : -Math.random() * 8,
            membersTrend:
              Math.random() > 0.5 ? Math.random() * 10 : -Math.random() * 5
          };
          ...
          const errorMessage =
            result.error?.message || result.message || '获取项目数据失败';
        ...
        const errorMessage =
          err instanceof Error ? err.message : '获取项目统计失败';
    ...
    <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
      {cards.map((card, i) => (
        <Card key={i} className='relative overflow-hidden'>
          ...
          {/* 背景装饰 */}
          <div
            className='from-muted/30 absolute top-0 right-0 bottom-0 w-32 bg-gradient-to-l to-transparent'
            aria-hidden='true'
          />
        </Card>
      ))}
