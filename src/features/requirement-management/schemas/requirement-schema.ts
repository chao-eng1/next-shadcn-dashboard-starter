import { z } from 'zod';
import { RequirementPriority, RequirementStatus, RequirementType, RequirementComplexity } from '../types/requirement';
...
export const requirementSortSchema = z.object({
  field: z.enum(['title', 'priority', 'status', 'createdAt', 'updatedAt', 'dueDate']),
...
export const requirementPaginationSchema = z.object({
  ...
  limit: z
    .string()
    .transform((val) => parseInt(val, 10))
    .pipe(z.number().min(1, '每页数量必须大于0').max(100, '每页数量不能超过100'))
    .default('20')
});
...
export const requirementImportSchema = z.object({
  ...
  priority: z
    ...
    .transform((val) => {
      ...
      if (Object.values(RequirementPriority).includes(upperVal as RequirementPriority)) {
  ...
  complexity: z
    ...
    .transform((val) => {
      const upperVal = val.toUpperCase();
      if (Object.values(RequirementComplexity).includes(upperVal as RequirementComplexity)) {
...
export const batchRequirementOperationSchema = z.object({
  ...
  operation: z.enum(['delete', 'updateStatus', 'updatePriority', 'updateAssignee']),
...
export const requirementSearchSchema = z.object({
  ...
  fields: z
    .array(z.enum(['title', 'description', 'acceptanceCriteria', 'businessValue', 'userStory']))
...
export type RequirementPaginationInput = z.infer<typeof requirementPaginationSchema>;
...
export type RequirementTemplateInput = z.infer<typeof requirementTemplateSchema>;
export type RequirementImportInput = z.infer<typeof requirementImportSchema>;
export type RequirementDependencyInput = z.infer<typeof requirementDependencySchema>;
export type RequirementStatusChangeInput = z.infer<typeof requirementStatusChangeSchema>;
export type BatchRequirementOperationInput = z.infer<typeof batchRequirementOperationSchema>;
