...
export function useRequirements({
  ...
  const fetchRequirements = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : '获取需求列表失败';
  ...
  const createRequirement = useCallback(
    async (data: RequirementFormData): Promise<Requirement> => {
      try {
        const response = await fetch(
        ...
        const errorMessage =
          err instanceof Error ? err.message : '创建需求失败';
        toast({
          title: '创建失败',
          description: errorMessage,
          variant: 'destructive'
  ...
  const deleteRequirement = useCallback(
    async (id: string): Promise<void> => {
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : '删除需求失败';
        toast({
          title: '删除失败',
          description: errorMessage,
          variant: 'destructive'
  ...
  const updateRequirementStatus = useCallback(
    async (id: string, status: RequirementStatus): Promise<void> => {
      try {
        const response = await fetch(
          `/api/projects/${projectId}/requirements/${id}`,
          {
