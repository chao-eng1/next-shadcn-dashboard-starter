...
export function useRequirementDetail({
  ...
  const fetchRequirement = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : '获取需求详情失败';
  ...
  const updateRequirement = useCallback(
    async (data: Partial<RequirementFormData>) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : '更新需求失败';
        toast({
          title: '更新失败',
          description: errorMessage,
          variant: 'destructive'
  ...
  const addComment = useCallback(
    async (content: string): Promise<RequirementComment> => {
  ...
  const uploadAttachment = useCallback(
    async (
      ...
      try {
        ...
        const response = await fetch(
          `/api/projects/${projectId}/requirements/${requirementId}/attachments`,
          {
            method: 'POST',
            body: formData
          }
        ...
        const errorMessage =
          err instanceof Error ? err.message : '上传附件失败';
        toast({
          title: '上传失败',
          description: errorMessage,
          variant: 'destructive'
  ...
  const deleteAttachment = useCallback(
    async (attachmentId: string) => {
      ...
      try {
        const response = await fetch(
          `/api/projects/${projectId}/requirements/${requirementId}/attachments/${attachmentId}`,
          {
            method: 'DELETE'
          }
        );
        ...
        const errorMessage =
          err instanceof Error ? err.message : '删除附件失败';
        toast({
          title: '删除失败',
          description: errorMessage,
          variant: 'destructive'
  ...
  const createVersion = useCallback(
    async (data: { changeReason: string }): Promise<RequirementVersion> => {
      if (!projectId || !requirementId) {
        throw new Error('Missing project or requirement ID');
      }
  ...
  const linkTask = useCallback(
    async (taskId: string) => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : '关联任务失败';
        toast({
          title: '关联失败',
          description: errorMessage,
          variant: 'destructive'
  ...
  const unlinkTask = useCallback(
    async (taskId: string) => {
      ...
      try {
        const response = await fetch(
          `/api/projects/${projectId}/requirements/${requirementId}/tasks/${taskId}`,
          {
            method: 'DELETE'
          }
        );
