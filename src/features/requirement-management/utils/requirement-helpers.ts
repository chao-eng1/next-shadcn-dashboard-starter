...
// 获取需求复杂度配置
export function getRequirementComplexityConfig(
...
// 格式化需求复杂度
export function formatRequirementComplexity(
  complexity: RequirementComplexity
): string {
...
// 获取需求优先级样式类名
export function getRequirementPriorityClassName(
  priority: RequirementPriority,
...
// 获取需求状态样式类名
export function getRequirementStatusClassName(
...
// 获取需求类型样式类名
export function getRequirementTypeClassName(
...
// 获取需求复杂度样式类名
export function getRequirementComplexityClassName(
  complexity: RequirementComplexity,
  variant: 'default' | 'badge' = 'default'
): string {
...
export function calculateRequirementProgress(
  ...
  const completedTasks = requirement.tasks.filter(
    (rt) => rt.task.status === 'DONE'
  ).length;
  const inProgressTasks = requirement.tasks.filter(
    (rt) => rt.task.status === 'IN_PROGRESS'
  ).length;
...
export function canEditRequirement(
  ...
  // 已完成或已取消的需求不能编辑
  if (
    [RequirementStatus.COMPLETED, RequirementStatus.CANCELLED].includes(
      requirement.status
    )
  ) {
    return false;
...
// 获取需求状态变更选项
export function getRequirementStatusTransitions(
...
// 生成需求层级路径
export function generateRequirementPath(
  requirement: RequirementWithRelations,
  allRequirements: RequirementWithRelations[]
): string[] {
  const path: string[] = [];
  ...
  while (current) {
    ...
    current = current.parentId
      ? allRequirements.find((r) => r.id === current!.parentId)
      : undefined;
  }
...
export function flattenRequirementTree(
  requirements: RequirementWithRelations[],
  level = 0
): Array<RequirementWithRelations & { level: number }> {
...
export function filterRequirements(
  ...
  return requirements.filter((req) => {
    ...
    if (
      ...
      !filters.type.includes(req.type)
    ) {
      return false;
    }
    ...
    if (
      ...
      !filters.complexity.includes(req.complexity)
    ) {
      return false;
    }
...
export function validateRequirementData(data: any): {
  ...
  if (
    ...
    (data.estimatedEffort < 0 || data.estimatedEffort > 1000)
  ) {
