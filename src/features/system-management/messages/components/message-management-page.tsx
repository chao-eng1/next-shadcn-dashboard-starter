...
export function MessageManagementPage({
  ...
  const [previewData, setPreviewData] = useState<MessageFormValues | null>(
    null
  );
  ...
  return (
    <div className='container mx-auto space-y-6 p-6'>
      {/* 成功提示区域 */}
      {showSuccessMessage && (
        <div className='animate-in slide-in-from-top-2 flex items-center gap-3 rounded-lg border border-green-200 bg-green-50 p-4 duration-300'>
                ...
                <ScrollArea className='h-[600px]'>
                  <div className='space-y-4'>
                    {messages.map((message) => {
                      const recipientInfo =
                        getRecipientTypeDescription(message);
                      const readCount = message.recipients.filter(
                        (r) => r.isRead
                      ).length;
