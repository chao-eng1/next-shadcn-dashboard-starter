        ...
        addMessage: (message) => {
          set((state) => {
            ...
            if (conversation) {
              ...
              // 如果不是自己发送的消息，增加未读计数
              if (!message.isOwn && message.conversationId !== state.selectedConversationId) {
...
export const useSelectedConversation = () => {
  ...
  const selectedId = useMessageStore(state => state.selectedConversationId);
...
export const useUnreadCounts = () => useMessageStore(state => ({
...
export const useDraft = (conversationId: string) => 
  useMessageStore(state => state.drafts[conversationId] || '');
export const useNotificationSettings = () => useMessageStore(state => state.notificationSettings);